...
function CreateApp() {
	...
	var coneArrays = tdl.primitives.createTruncatedCone(0.4, 0.0, 0.8, 12, 1,
			true, true);

	// Load textures
	var textures = {
		diffuseSampler : tdl.textures
				.loadTexture('resource/images/happy-face.png')
	};

	// Create Shader Program
	var program = tdl.programs.loadProgramFromScriptTags('sphereVertexShader',
			'sphereFragmentShader');

	// --Setup Models---------------------------------------
	var models = [ new tdl.models.Model(program, sphereArrays, textures),
	...
	for (var ii = 0; ii < g_maxObjects; ++ii) {
		instances.push({
			...
			y : 0,
			z : 0,
			colorMult : new Float32Array([ Math.random()+0.3, Math.random()+0.3,
					Math.random()+0.3, 1 ]),
			modelIndex : Math.floor(ii) % models.length,
	...
	function renderBegin() {
		...
		m4.perspective(projection, math.degToRad(60), canvas.clientWidth
				/ canvas.clientHeight, 1, 5000);
...
Test.prototype.initTest = function() {	
	// support check
	if (!Modernizr.webgl) {
		logger.log('webgl is not supported');
		this.config.result = 'unsupported';
		return false;
	}
	...
	this.initialize();
	this.app = CreateApp();
	g_numObjects = this.config.stress * 10;
};
